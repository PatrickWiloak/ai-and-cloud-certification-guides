...
- **Authentication**: vLLM has no auth by default. Put it behind nginx, an API gateway, or a tiny reverse proxy that validates a header.
- **Health endpoint**: `/health`. Wire to your load balancer.
- **Container**: `vllm/vllm-openai:latest` Docker image is the standard production deploy.
